...
import { GlobeIcon, ListIcon, MagnifyingGlassIcon, SignOutIcon, UserCircleIcon } from '@phosphor-icons/react'
import { InputGroup, InputGroupAddon, InputGroupInput } from 'ui/ui/input-group'
import { Button } from "ui/ui/button";
import { LogoText } from "./logo";
import { DropdownMenu, DropdownMenuContent, DropdownMenuGroup, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from "ui/ui/dropdown-menu";
import { useIsMobile } from 'ui/hooks/use-is-mobile.ts'
import { Select, SelectContent, SelectGroup, SelectItem, SelectTrigger, SelectValue } from "ui/ui/select";
...
const DesktopNav: FC = () => <> <nav className="flex items-center col-span-3 gap-6">
    <Link to="/">
        <LogoText />
    </Link>
                ...
                <SelectGroup>
                    {
                        languages.map(({
                            ...
                            <SelectItem key={code} value={code}>{value}</SelectItem>
                        )
                    }
                </SelectGroup>
            </SelectContent>
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
                <DropdownMenuGroup>
                    ...
                    <DropdownMenuItem className='w-full cursor-pointer' render={<Link
                        to="/"
                    />}>
                ...
                <DropdownMenuGroup>
                    <DropdownMenuItem className='w-full' render={<Button
                        size='lg'
                        variant='destructive'
                    />}>
                        <SignOutIcon /> Logout
...
const MainHeader: FC = () => {
    ...
    return <header className="p-2 container mx-auto grid-cols-7 gap-2 md:grid">
        {
